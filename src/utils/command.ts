...
export function safeJsonParse<T>(
  ...
  try {
    ...
    // Provide a concise, non-noisy warning for failed JSON parse.
    // Include a small snippet of the input (first 200 chars) and the error message to aid debugging
    const inputSnippet = String(jsonString).slice(0, 200)
    const errMsg = error instanceof Error ? error.message : String(error)
    console.warn(
      `safeJsonParse: failed to parse JSON (first 200 chars): "${inputSnippet}"... - ${errMsg}`,
    )
    return defaultValue
  }
}
